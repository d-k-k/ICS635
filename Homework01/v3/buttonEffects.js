...
function updateSettingsBasedOnInput() {
	...
	settings.tldfo = parseFloat(document.getElementById("inputTldfo").value);
...
function getVarianceSettings() {
	...
	else if (document.getElementById("radialVarTldfo").checked) { whichVariableToChange = "tldfo";
		perceptron.varianceControls.currentValue = parseFloat(document.getElementById("inputTldfo").value);}
	...
	if (whichVariableToChange === null) {
		...
		perceptron.varianceControls.log = 
			...
			+ "w0, n, lr, r, m, iteration, dist of tl from origin\n";
	}
...
function shouldKeepData() {
	...
	return document.getElementById("keepData").checked;
}
